...
- **PBKDF2 password hashing**: Node-native Web Crypto API PBKDF2 (100k vòng băm) với constant-time verification
...
```env
CMS_USER=admin
CMS_PASS_HASH="pbkdf2:100000:..."   # Mã băm PBKDF2 Web Crypto (dùng bin/cli.js để tạo)
...
```bash
# Tạo mã băm bằng Web Crypto API PBKDF2 (Node.js native):
node bin/cli.js hash mat-khau-cua-ban
```

Copy kết quả băm nhận được (dạng `pbkdf2:100000:salt_hex:hash_hex`) vào `CMS_PASS_HASH` trong file `.env` (bọc trong dấu ngoặc kép).
